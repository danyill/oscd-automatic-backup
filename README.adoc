...
:uri-plugin: https://github.com/danyill/oscd-automatic-backup
:uri-ci-bundle: https://danyill.github.io/oscd-automatic-backup/oscd-automatic-backup.js
:uri-material-icons: https://fonts.google.com/icons
:uri-openscd-core-plugin-demo: https://danyill.github.io/oscd-automatic-backup/index.deploy.html
...
* The user chooses the number and frequency of backups and allows the plugin to write to a specific folder.
Backups are then taken when the plugin is active with the oldest removed first to ensure the number of backups does not exceed selected amount.
+
CAUTION: The File System Access API is relatively new and as of October 2023 is not supported in Firefox, Safari or in any mobile web browsers.
...
For a local resource, begins without a forward slash, e.g. `plugins/oscd-automatic-backup/dist/oscd-automatic-backup.js`. 
...
----
{
  ...
  "active": true, // <.>
  "requireDoc": true, // <.>
  "src": "/dist/oscd-automatic-backup.js" // <.>
}
----
<.> Name of the plugin which appears in the editor menu at top of screen
...
<.> Whether a document must be loaded for this plugin to be available
<.> URL which can be a local or remote resource. 
For a local resource, begins with a forward slash, e.g. `plugins/oscd-automatic-backup/dist/oscd-automatic-backup.js`. 
